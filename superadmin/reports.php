...
$inv_page = isset($_GET['inv_page']) ? max(1, (int)$_GET['inv_page']) : 1;
$inv_per_page = 50;
                                ...
                                <div class="modal-body">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-6 rpci-certification-only">
                                            ...
                                            <input type="text" name="rpci_verified_name" class="form-control" value="MS. SIMONETTE D. CATALUÃ‘A">
                                        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            <form method="GET" id="inventoryTxFilterForm">
                                ...
                                <input type="hidden" name="inv_page" id="inv_page" value="<?php echo (int)$inv_page; ?>">
                                <div class="row g-3 align-items-end">
                                    <div class="col-md-3">
                                        <label class="form-label text-secondary fw-semibold small">Period</label>
                                        <select name="inv_period" id="inv_period" class="form-select" onchange="this.form.inv_page.value=1; this.form.submit()">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select name="tx_type" id="tx_type" class="form-select" onchange="this.form.inv_page.value=1; this.form.submit()">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover align-middle">
                                ...
                                <tbody>
                                    <?php
                                    $start_esc = $conn->real_escape_string($tx_start);
                                    $end_esc = $conn->real_escape_string($tx_end);
                                    $type_esc = $tx_type !== '' ? $conn->real_escape_string($tx_type) : '';
                                    ...
                                    $sql_inventory_tx = "SELECT
                                        ...
                                        WHERE DATE(t.transaction_date) BETWEEN '$start_esc' AND '$end_esc'";

                                    if ($type_esc !== '') {
                                        $sql_inventory_tx .= " AND i.item_type = '$type_esc'";
                                    }

                                    $count_inventory_sql = "SELECT COUNT(*) AS total_count
                                        FROM inventory_transactions t
                                        JOIN items i ON t.item_id = i.id
                                        WHERE DATE(t.transaction_date) BETWEEN '$start_esc' AND '$end_esc'";
                                    if ($type_esc !== '') {
                                        $count_inventory_sql .= " AND i.item_type = '$type_esc'";
                                    }

                                    $total_inventory = 0;
                                    $count_inventory_result = $conn->query($count_inventory_sql);
                                    if ($count_inventory_result && $count_inventory_result->num_rows > 0) {
                                        $row_count = $count_inventory_result->fetch_assoc();
                                        $total_inventory = (int)$row_count['total_count'];
                                    }

                                    $total_pages = $total_inventory > 0 ? (int)ceil($total_inventory / $inv_per_page) : 1;
                                    if ($inv_page > $total_pages) {
                                        $inv_page = $total_pages;
                                    }
                                    $offset = ($inv_page - 1) * $inv_per_page;

                                    // Find first transaction in the period per (item_id + description)
                                    $beginning_balances = [];
                                    $begin_sql = "SELECT
                                            i.id AS item_id,
                                            i.description,
                                        ...
                                        WHERE DATE(t.transaction_date) BETWEEN '$start_esc' AND '$end_esc'";
                                    if ($type_esc !== '') {
                                        $begin_sql .= " AND i.item_type = '$type_esc'";
                                    }
                                    ...
                                    if ($begin_result && $begin_result->num_rows > 0) {
                                        while ($row_begin = $begin_result->fetch_assoc()) {
                                            if (!empty($row_begin['first_tx_date'])) {
                                                ...
                                                $beginning_balances[$key] = [
                                                    'first_tx_date' => $row_begin['first_tx_date'],
                                                    'begin_qty' => calculate_beginning_balance($conn, (int)$row_begin['item_id'], $tx_start)
                                                ];
                                            }
                                        }
                                    }

                                    $sql_inventory_tx .= " ORDER BY i.item ASC, i.description ASC, t.transaction_date ASC LIMIT $inv_per_page OFFSET $offset";
                                    ...
                                    if ($result_inventory_tx && $result_inventory_tx->num_rows > 0) {
                                        ...
                                        while ($tx = $result_inventory_tx->fetch_assoc()) {
                                            ...
                                            $qty = (int)$tx['quantity'];
                                            $end_bal = (int)$tx['balance_after'];
                                            $tx_type_lower = strtolower((string)$tx['transaction_type']);
                                            ...
                                            if (isset($beginning_balances[$key]) && $beginning_balances[$key]['first_tx_date'] === $tx['transaction_date']) {
                                                $begin_date_label = date('M d, Y', strtotime($tx_start));
                                                $remarks_text = 'Beginning balance as of ' . $begin_date_label . ' are ' . $instocks_before . ' in-stocks';
                                            }
                                            ?>
                                            <tr>
                                                <td><?php echo $isNewItem ? htmlspecialchars($tx['item']) : ''; ?></td>
                                                ...
                                                <td><span class="fw-bold"><?php echo htmlspecialchars($remarks_text); ?></span></td>
                                            </tr>
                                            <?php
                                            $lastItem = $tx['item'];
                                        }
                            ...
                            <?php
                            if ($total_inventory > 0 && $total_pages > 1) {
                                $base_query = [
                                    'rt' => 'inventory_system',
                                    'inv_period' => $tx_period
                                ];
                                if ($type_esc !== '') {
                                    $base_query['tx_type'] = $tx_type;
                                }
                                if ($tx_period === 'range' && isset($_GET['start_date'], $_GET['end_date'])) {
                                    $base_query['start_date'] = $_GET['start_date'];
                                    $base_query['end_date'] = $_GET['end_date'];
                                }
                                ?>
                                <nav class="mt-3">
                                    <ul class="pagination justify-content-end mb-0">
                                        <?php for ($p = 1; $p <= $total_pages; $p++): ?>
                                            <?php
                                            $base_query['inv_page'] = $p;
                                            $url = 'reports.php?' . http_build_query($base_query);
                                            $active = $p === $inv_page ? ' active' : '';
                                            ?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (risSearchInput) {
            ...
            if (rpciPane) {
                ...
                const rpciRows = rpciPane.querySelectorAll('tbody tr[data-item-type]');
                const isSlc = selectedType === 'slc';
                const isIcsOrScOrRpci = selectedType === 'ics' || selectedType === 'sc' || selectedType === 'rpci';

                if (selectAllRpci) {
                    // Enable select all for all reports in this section
                    selectAllRpci.disabled = false;
                }

                rpciRows.forEach(function(row) {
                    const itemType = (row.getAttribute('data-item-type') || '').toLowerCase();
...
<script>
    ...
    function printInventoryTransactions() {
        // Use PHP-computed dates so Annual/Weekly/Daily periods are correct
        const startDate = '<?php echo addslashes($tx_start); ?>';
        const endDate   = '<?php echo addslashes($tx_end); ?>';
    ...
    // Ensure collapse buttons work properly
