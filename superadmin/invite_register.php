                ...
                try {
                    ...
                    if ($username_error !== '') {
                        ...
                        if ($has_profile_picture_col) {
                            $insert = $conn->prepare('INSERT INTO users (username, first_name, middle_name, last_name, password, role, email, phone, address, profile_picture, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())');
                        } else {
                            $insert = $conn->prepare('INSERT INTO users (username, first_name, middle_name, last_name, password, role, email, phone, address, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())');
                        }
                        if (!$insert) {
                            throw new Exception('Error preparing account creation: ' . $conn->error);
                        }
                        if ($has_profile_picture_col) {
                            $insert->bind_param('ssssssssss', $username, $first_name, $middle_name, $last_name, $hashed_password, $role, $email, $phone, $address, $profile_picture_name);
                        } else {
                            $insert->bind_param('sssssssss', $username, $first_name, $middle_name, $last_name, $hashed_password, $role, $email, $phone, $address);
                        }
                        if (!$insert->execute()) {
